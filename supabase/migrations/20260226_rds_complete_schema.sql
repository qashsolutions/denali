-- =============================================================================
-- Denali Health — Complete AWS RDS Schema
-- =============================================================================
-- Generated: 2026-02-26
-- Target: AWS RDS PostgreSQL 17 (denali-prod.ca5m0qc8e5h8.us-east-1.rds.amazonaws.com)
-- User: denali_admin / Database: denali
--
-- APPLY WITH:
--   psql "host=denali-prod.ca5m0qc8e5h8.us-east-1.rds.amazonaws.com \
--         dbname=denali user=denali_admin sslmode=require" \
--         -f 20260226_rds_complete_schema.sql
--
-- WHAT THIS FILE DOES:
--   - Creates all 35 public application tables
--   - Creates all 5 latest views (denial codes, CARC/RARC/EOB, appeal levels)
--   - Creates flywheel_metrics materialized view
--   - Creates all 35+ stored functions and procedures
--   - Creates all primary keys, unique constraints, foreign keys
--   - Creates all performance indexes
--   - Creates triggers for updated_at columns
--   - Adds OTP + TOTP columns to user_verification (Phase 2 auth migration)
--
-- WHAT THIS FILE SKIPS (Supabase-specific):
--   - auth.*, realtime.*, storage.*, vault.* schemas and tables
--   - ROW LEVEL SECURITY policies (auth done server-side via admin DB user)
--   - auth.users foreign key on public.users (no auth schema on RDS)
--   - Supabase extensions: pg_net, hypopg, index_advisor, pg_graphql, supabase_vault
--   - handle_new_user trigger (Cognito handles user creation)
--   - custom_access_token_hook (Supabase JWT hook)
--   - is_admin() (uses auth.uid())
-- =============================================================================
...
CREATE EXTENSION IF NOT EXISTS pgcrypto;   -- gen_random_bytes, crypt (used in some functions)
...
-- appeal_levels
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.appeal_levels (
...
-- ------------------------------------------------------------
-- appeal_outcomes (no user link — anonymous learning data)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- audit_logs (HIPAA 6-year retention — never cascade-delete)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- carc_codes (versioned by effective_date)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.carc_codes (
    code text NOT NULL,
    group_code text,
    description text NOT NULL,
    category text,
...
-- ------------------------------------------------------------
-- chat_daily_usage (rate limiting)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- conversation_patterns (learning)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- coverage_paths (learning)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.coverage_paths (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
-- ------------------------------------------------------------
-- denial_patterns (versioned by effective_date)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.denial_patterns (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
-- diabetes_insights
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.diabetes_insights (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    user_id uuid NOT NULL,
...
-- diabetes_log
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.diabetes_log (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    user_id uuid NOT NULL,
...
-- ------------------------------------------------------------
-- diabetes_snapshots (append-only longitudinal lab history)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.diabetes_snapshots (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    user_id uuid NOT NULL,
...
-- ------------------------------------------------------------
-- eob_denial_mappings (versioned by effective_date)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.eob_denial_mappings (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
-- ------------------------------------------------------------
-- fhir_cache (Blue Button FHIR data, 24h TTL)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.fhir_cache (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    user_id uuid NOT NULL,
...
-- ------------------------------------------------------------
-- landing_content (CMS-managed landing page sections)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- learning_queue (async learning job processor)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- outcome_followups (appeal outcome tracking + incentive)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- policy_cache (Medicare policy tracking + change detection)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- pricing_plans (Stripe plan config)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- procedure_mappings (learning: phrase → CPT)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.procedure_mappings (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
-- ------------------------------------------------------------
-- provider_practices (counselor/provider dashboard)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- rarc_codes (versioned by effective_date)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.rarc_codes (
    code text NOT NULL,
    description text NOT NULL,
    category text,
    plain_english text,
...
-- ------------------------------------------------------------
-- symptom_mappings (learning: phrase → ICD-10)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- usage (appeal count + credits per email)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- user_verification (email OTP + TOTP MFA — Phase 2 AWS auth)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- users (primary user table — id = Cognito sub UUID)
-- ------------------------------------------------------------
...
-- =============================================================================
-- FOREIGN KEY CONSTRAINTS (all within public schema)
-- =============================================================================
...
-- OTP expiry cleanup index (Phase 2)
CREATE INDEX idx_user_verification_otp_expires ON public.user_verification (otp_expires_at)
  WHERE otp_expires_at IS NOT NULL;

-- =============================================================================
-- VIEWS (versioned denial code tables — latest effective_date)
-- =============================================================================

CREATE VIEW public.appeal_levels_latest AS
 SELECT id, level, name, description, time_limit, decision_timeframe, success_rate, effective_date, created_at
   FROM public.appeal_levels
  WHERE (effective_date = ( SELECT max(al.effective_date) FROM public.appeal_levels al))
  ORDER BY level;

CREATE VIEW public.carc_codes_latest AS
 SELECT code, group_code, description, category, plain_english, effective_date, is_active, created_at
   FROM public.carc_codes
  WHERE (effective_date = ( SELECT max(cc.effective_date) FROM public.carc_codes cc));

CREATE VIEW public.rarc_codes_latest AS
 SELECT code, description, category, plain_english, effective_date, is_active, created_at
   FROM public.rarc_codes
  WHERE (effective_date = ( SELECT max(rc.effective_date) FROM public.rarc_codes rc));

CREATE VIEW public.eob_denial_mappings_latest AS
 SELECT id, eob_code, eob_description, carc_code, rarc_code, effective_date, created_at
   FROM public.eob_denial_mappings
  WHERE (effective_date = ( SELECT max(edm.effective_date) FROM public.eob_denial_mappings edm));

CREATE VIEW public.denial_patterns_latest AS
 SELECT id, reason, category, reason_codes, common_cpts, common_diagnoses, appeal_strategy,
        documentation_checklist, estimated_success_rate, appeal_deadline_days, effective_date, is_active, created_at
   FROM public.denial_patterns
  WHERE ((effective_date = ( SELECT max(dp.effective_date) FROM public.denial_patterns dp)) AND (is_active = true));
...
CREATE MATERIALIZED VIEW public.flywheel_metrics AS
 SELECT a.cpt_codes, a.icd10_codes, a.lcd_refs, carc.carc_code, a.status AS outcome,
  ...
  GROUP BY a.cpt_codes, a.icd10_codes, a.lcd_refs, carc.carc_code, a.status,
           (EXTRACT(epoch FROM (a.outcome_reported_at - a.created_at)) / (86400)::numeric), u.counselor_state
  WITH NO DATA;
...
CREATE OR REPLACE FUNCTION public.check_and_increment_chat(p_identifier text, p_daily_limit integer) RETURNS jsonb
...
-- Appeal credits: add (Stripe single payment fulfillment)
CREATE OR REPLACE FUNCTION public.add_appeal_credits(p_email text, p_credits integer) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  UPDATE public.usage
...
-- Appeal credits: decrement (used on appeal save)
CREATE OR REPLACE FUNCTION public.decrement_appeal_credit(p_email text) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_credits INT;
...
-- Appeal credits: reset monthly (Stripe subscription renewal)
CREATE OR REPLACE FUNCTION public.reset_monthly_appeal_credits(p_email text, p_credits integer) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  UPDATE public.usage
...
-- Appeal count: increment (lifetime counter)
CREATE OR REPLACE FUNCTION public.increment_appeal_count(p_email text, p_user_id uuid DEFAULT NULL::uuid, p_device_fingerprint text DEFAULT NULL::text) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_count INT;
...
-- Appeal access check (legacy — client uses appeal_credits directly)
CREATE OR REPLACE FUNCTION public.check_appeal_access(p_email text DEFAULT NULL::text) RETURNS text
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_credits INT;
...
-- Appeal count getter
CREATE OR REPLACE FUNCTION public.get_appeal_count(p_email text) RETURNS integer
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_count INT;
...
-- Outcome incentive: apply free credit on outcome report
CREATE OR REPLACE FUNCTION public.apply_outcome_incentive(p_email text) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
-- Stripe: handle subscription lifecycle events
...
-- Claim conversation: assign anonymous conversation to user on sign-in
-- AWS version: takes explicit p_user_id (Supabase used auth.uid())
CREATE OR REPLACE FUNCTION public.claim_conversation(p_conversation_id uuid, p_user_id uuid DEFAULT NULL::uuid) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_updated BOOLEAN := false;
...
-- Search denial codes: full-text search across CARC + RARC
CREATE OR REPLACE FUNCTION public.search_denial_codes(search_text text) RETURNS TABLE(code_type text, code text, description text, category text, plain_english text)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
-- Denial pattern for CARC code
CREATE OR REPLACE FUNCTION public.get_denial_pattern_for_carc(carc_code_input text)
    RETURNS TABLE(reason text, category text, appeal_strategy text, documentation_checklist text[], estimated_success_rate text, appeal_deadline_days integer)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
-- Denial patterns for CPT code
CREATE OR REPLACE FUNCTION public.get_denial_patterns_for_cpt(cpt_code_input text)
    RETURNS TABLE(id uuid, reason text, category text, reason_codes text[], common_cpts text[], common_diagnoses text[], appeal_strategy text, documentation_checklist text[], estimated_success_rate text, appeal_deadline_days integer, effective_date date, is_active boolean, created_at timestamp with time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
-- Grouped audit logs for Settings "Data Access History"
CREATE OR REPLACE FUNCTION public.get_grouped_audit_logs(p_user_id uuid, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0)
    RETURNS TABLE(action text, resource_type text, ip_address text, latest_at timestamp with time zone, log_date date, entry_count integer)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
-- Unreported outcome for returning users
CREATE OR REPLACE FUNCTION public.get_unreported_outcome(p_email text)
    RETURNS TABLE(appeal_id uuid, followup_id uuid, followup_type text, scheduled_at timestamp with time zone, service_description text, denial_date text, created_at timestamp with time zone)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
  SELECT
    ...
    a.denial_date::text,
    f.created_at
  FROM public.outcome_followups f
  JOIN public.appeals a ON f.appeal_id = a.id
...
CREATE OR REPLACE FUNCTION public.record_appeal_outcome(
    ...
    p_email text,
    p_outcome text,
    p_denial_reason text DEFAULT NULL::text,
    p_documentation_gaps text[] DEFAULT NULL::text[],
    p_successful_arguments text[] DEFAULT NULL::text[],
    p_days_to_resolution integer DEFAULT NULL::integer
) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
-- Process feedback: thumbs up/down
CREATE OR REPLACE FUNCTION public.process_feedback(
    p_message_id uuid,
    p_user_id uuid DEFAULT NULL::uuid,
    p_rating text DEFAULT NULL::text,
    p_correction text DEFAULT NULL::text,
...
-- Learning: update symptom mapping
CREATE OR REPLACE FUNCTION public.update_symptom_mapping(p_phrase text, p_icd10_code text, p_icd10_description text, p_boost real) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
-- Learning: update procedure mapping
CREATE OR REPLACE FUNCTION public.update_procedure_mapping(p_phrase text, p_cpt_code text, p_cpt_description text, p_boost real) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
-- Learning: update coverage path
CREATE OR REPLACE FUNCTION public.update_coverage_path(
    p_icd10_code text,
    p_cpt_code text,
    p_outcome text,
    p_ncd_id text DEFAULT ''::text,
    p_lcd_id text DEFAULT ''::text,
    p_contractor_id text DEFAULT NULL::text,
    p_documentation_required text[] DEFAULT NULL::text[]
) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
-- Learning: update conversation pattern
CREATE OR REPLACE FUNCTION public.update_conversation_pattern(p_trigger_phrase text, p_intent text, p_question_sequence jsonb, p_was_successful boolean) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
-- Learning: get learned context for prompts
CREATE OR REPLACE FUNCTION public.get_learning_context(
    p_symptom_phrases text[] DEFAULT NULL::text[],
    p_icd10_codes text[] DEFAULT NULL::text[],
    p_cpt_codes text[] DEFAULT NULL::text[],
...
-- Learning: get appeal context
CREATE OR REPLACE FUNCTION public.get_appeal_context(p_icd10_codes text[], p_cpt_codes text[]) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_result jsonb := '{}'::jsonb;
...
-- Learning: prune weak mappings
CREATE OR REPLACE FUNCTION public.prune_weak_mappings(
    p_min_confidence real DEFAULT 0.3,
    p_min_use_count integer DEFAULT 2,
    p_days_inactive integer DEFAULT 90
) RETURNS TABLE(pruned_symptoms integer, pruned_procedures integer, pruned_coverage_paths integer)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
-- Learning: queue job
CREATE OR REPLACE FUNCTION public.queue_learning_job(p_job_type text, p_job_data jsonb, p_priority integer DEFAULT 5) RETURNS uuid
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_id UUID;
...
-- Learning: claim job (for edge function processor)
CREATE OR REPLACE FUNCTION public.claim_learning_job()
    RETURNS TABLE(job_id uuid, job_type text, job_data jsonb)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_id UUID;
...
-- Learning: complete job
CREATE OR REPLACE FUNCTION public.complete_learning_job(p_job_id uuid, p_success boolean, p_error text DEFAULT NULL::text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  UPDATE public.learning_queue
...
CREATE OR REPLACE FUNCTION public.track_user_event(
    ...
    p_appeal_id uuid DEFAULT NULL::uuid,
...
-- Counselor: case ref generator
CREATE OR REPLACE FUNCTION public.generate_case_ref(p_counselor_id uuid, p_initials text) RETURNS text
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
  v_count INT;
...
-- Counselor: stats
CREATE OR REPLACE FUNCTION public.get_counselor_stats(p_counselor_id uuid)
    RETURNS TABLE(open_cases bigint, filed_this_month bigint, outcomes_reported bigint, approved_count bigint, denied_count bigint, partial_count bigint, avg_resolution_days numeric)
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
  RETURN QUERY
...
-- Flywheel metrics: refresh materialized view
CREATE OR REPLACE FUNCTION public.refresh_flywheel_metrics() RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
...
-- Flywheel: get context for CARC codes
...
-- User cascade delete (GDPR/CCPA)
CREATE OR REPLACE FUNCTION public.delete_user_cascade(target_user_id uuid) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
BEGIN
