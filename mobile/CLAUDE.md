...
- **A11y 45+ floor: body ≥17px, touch targets ≥48px (D35).** `tokens.ts` `sizes.base` is 17 (Dynamic Type still respected — MAX_FONT_SCALE caps chrome only). Interactive `minHeight`/`minWidth` are ≥48 — NEVER re-introduce a 44px target (a source-scan test `theme/__tests__/a11y.test.ts` fails on `min(Height|Width): 44`). The font bump is colors-test-neutral (token-drift asserts the palette, not sizes).
- **Bottom-tab ACTIVE tint is `tealDeep`, never bare `teal` (A11Y #6).** React Navigation's `tabBarActiveTintColor` colors the selected tab's LABEL (10.5px text), not just its icon, so it must clear WCAG AA *as text*: bare `teal` on the white tab surface is 4.11:1 (sub-AA), `tealDeep` is 6.35:1 light / 8.20:1 dark — the same teal-text rule as the A11Y-01/02/1b consumers. `src/navigation/MainTabs.tsx` uses `redesign.tealDeep`; a source-scan pin in `theme/__tests__/tokens.test.ts` (A11Y #6) fails CI on any revert to `teal`. Don't split the tint to re-`teal` the icon — the unified `tealDeep` keeps icon+label consistent and both clear their floors (a non-text icon needs only 3:1).
- **Today surfaces ONE attention card, never a pile-up (D35).** The dashboard pins a single compact callout — a **light-grey (`pillSoft`) card with a thin `alarm` border, no icon** — listing the **top 4 severe domains** as borderless, individually-tappable red text chips in **one row that never wraps** (`flexWrap: "nowrap"` + chip `numberOfLines={1}` + `flexShrink: 1`), with a "+N" overflow (a11y-labelled "N more in the severe range") when more than 4 are severe. `severeDomains` (`timeline/dashboardAttention.ts`) returns ALL `alarm`-tint instrument domains; the render `slice(0, 4)`s them. It reuses the cards' own `lookupInterpretation` + `tintClassForBand` (the RN-free `bandTint.ts`) so callout and card can never disagree, reuses the versioned domain name + band pill — NO new clinical copy — and the grid below is NOT reordered. Each chip → its domain detail where the full reading + any 988 path live (chips use vertical `hitSlop` to reach the 48px tap floor without horizontal overlap). Don't restore the single-domain `mostSevereDomain` form, don't stack N cards, don't add advice copy. Markers (watch tint) never trigger it.
...
- **The markers detail shows ONE card per marker (latest), never card-per-entry; the CHART carries history (D27).** `health_markers` cards are deduped to latest-per-code via the pure `latestPerCode` (`src/screens/markers/latestPerCode.ts`) under a "Latest readings" label — NOT date-bucketed. The dedup feeds the CARD list ONLY; `BmiTrendChart` (and any future per-marker chart) is ALWAYS passed the full unfiltered `mine.rows` so every reading still plots as a dot — never feed `latestPerCode` output to a chart. Scope is `health_markers` ONLY: `health_history` (distinct diagnoses) and instrument domains keep the all-entries date-bucketed view. Display-only — storage/export keep every reading. Older readings of non-charted markers are intentionally not shown as cards (retained in storage; "show all readings" expander is the planned follow-up).
...
- **Settings "Your details": gender + Medicare editable; year + sex PERMANENT (D31/D32).** `SettingsScreen` shows year-of-birth (+age), sex at birth, gender identity, Medicare from the local profile. **Gender identity + Medicare are editable in-place** (tap-to-expand picker → `upsertProfile({id, email, ...patch})`; local write, no sign-out, NO data loss — `signOut()` keeps the encrypted DB). **Year of birth + sex at birth are LOCKED** (greyed + lock icon, no edit affordance) — they're clinical interpretation keys; changing them would re-interpret all existing results, so they're set once and confirmed permanent at sign-up (D33). NEVER add an edit path for sex_at_birth / birth_year without the clinical-change handling. Labels + the gender option list live in the pure `demographicsDisplay` helper.
- **Onboarding has a CONFIRM step (5 of 5) for the permanent fields (D33).** `CohortOnboardingScreen` ends with a confirm step that summarizes year-of-birth + sex-at-birth and makes the user acknowledge they can't be changed before the profile is written (gender selection / Skip advance to it; `confirmAndSubmit` does the write). Don't collapse it back into auto-submit — it's the informed-consent gate that justifies locking those fields in Settings. `cohort_confirm_summary` + the confirm `oneitem_continue_button` are in the `signin_onboarding` / `crisis_988` Maestro flows.
- **Instrument details show the LATEST check-in card + chart, history drills down (D30).** Instrument domains (Mood/Anxiety/Sleep/Alcohol/Urinary/Menopause/Hormonal) render the trend chart (every check-in = a dot) + ONE "Latest check-in" card; the full session list lives in `InstrumentHistory` (tap the card → "View past check-ins"). Don't restore the card-per-session wall on the domain detail. The chartable-instrument set MUST be computed from ALL sessions (`instrumentDomainView`), never the single rendered card — else a domain whose latest session is the non-chartable PHQ-2 gate loses its chart; a pinned test guards this. PHQ-2 stays gate-only (never charted). `health_history` keeps its full diagnosis list (distinct facts, not consolidated). `InstrumentSessionCard`/`SingleRowCard` `onPress` is opt-in (navigable card) — absent = byte-identical expand; `InstrumentHistory` omits it so per-session detail stays expandable there.
- **Per-marker history is a DRILL-DOWN; raw markers chart BAND-LESS (D28).** Tapping a "Latest readings" card opens `MarkerDetail` ({code}) — `MarkerTrendChart` + "All readings" list + provenance. `MarkerTrendChart` shades bands ONLY from `INTERPRETATION_TABLE_V1.biomarkers[code]` (`resolveMarkerBands`) — a marker with no table entry renders BAND-LESS (plain line + `yRefs` value gridlines), NEVER an invented band (a raw value with no validated cutoffs makes no severity claim). `TrendChartSvg`'s `yRefs` is OPTIONAL; banded charts (instruments, BMI) MUST keep omitting it so their render stays byte-identical. Axis/points/yRefs math lives in the pure, pinned `markerChartModel.ts` — keep it there, not inline in the component. `TimelineCardView`'s `onPress` is opt-in (navigable card); absent = byte-identical expand behavior — don't make it default. The drill-down is read-only: never write the DAL from these screens.
- **BMI obesity is split into class I-II + class III (D29).** `tableV1` ships FIVE BMI bands: underweight / healthy-weight / overweight / obesity (30.0–39.9) / **severe-obesity (≥40.0)** — WHO TRS 894 classes. Don't re-collapse them. `severe-obesity` is `watch` tint (WHO out-of-range bands stay calm; reviewer escalates via per-band `tint` before sign-off), `provisional: true`, ‡ until a named clinician clears it. Adding/auditing BMI bands keeps the 1-dp contiguity (39.9 / 40.0) so no value mis-bands.
