...
- **Settings "Your details" shows demographics READ-ONLY; editing is safe but clinically guarded (D31).** `SettingsScreen` displays year-of-birth (+age), sex at birth, gender identity, Medicare from the local profile (`getProfile`), greyed/read-only. Labels via the pure `demographicsDisplay` helper. Editing demographics does NOT need sign-out and does NOT lose data (`upsertProfile` is a local write; `signOut()` keeps the encrypted DB) — so read-only is a v1 choice, not a constraint. When in-place editing lands: `sex_at_birth` + `birth_year` are clinical interpretation keys (changing them re-interprets existing results + re-gates cohort markers) → require a deliberate confirm/warning; `gender_identity` is display-only (free to edit). Never make `sex_at_birth` casually editable without the clinical-change warning.
