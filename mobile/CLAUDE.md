...
- **Today surfaces ONE attention item, never a pile-up (D35).** The dashboard pins a single callout for the most-severe domain (`alarm`-tint latest band) via `mostSevereDomain` (`timeline/dashboardAttention.ts`), reusing the cards' own `lookupInterpretation` + `tintClassForBand` (the RN-free `bandTint.ts`). It reuses the versioned band pill — NO new clinical copy — and the grid below is NOT reordered. Don't turn this into multiple alerts or add advice copy; tap routes to the detail where the 988 path lives.
