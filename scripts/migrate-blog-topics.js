#!/usr/bin/env node
// Blog topics migration: user_topic_preferences table + 6 new posts + tag updates
// Run inside ECS container: node /tmp/migrate-blog-topics.js
...
const TAG_EXISTING_SQL = `
UPDATE blog_posts SET tags = ARRAY['medicare-general']
WHERE tags = '{}' OR tags IS NULL;
`;

const SEED_SQL = `INSERT INTO blog_posts (slug, title, kicker, key_message, body, category, cta_text, cta_url, sources, tags, meta_title, meta_description, published, published_at) VALUES
...
async function main() {
  try {
    ...
    // Step 1: Create user_topic_preferences table
    console.log('--- Step 1: Create user_topic_preferences table ---');
    await client.query(CREATE_TABLE_SQL);
    console.log('Result: table + index created\n');

    // Step 2: Tag existing posts with medicare-general
    console.log('--- Step 2: Tag existing blog posts ---');
    const tagResult = await client.query(TAG_EXISTING_SQL);
    console.log(`Result: ${tagResult.rowCount} posts tagged with medicare-general\n`);

    // Step 3: Seed new diabetes + obesity posts
    console.log('--- Step 3: Seed new blog posts ---');
    const seedResult = await client.query(SEED_SQL);
    console.log(`Result: INSERT — Rows: ${seedResult.rowCount}\n`);

    // Step 4: Verify
    console.log('--- Verification ---');

    const countResult = await client.query('SELECT COUNT(*) as total FROM blog_posts');
    console.log(`Total blog posts: ${countResult.rows[0].total}`);
    ...
    `);
    console.log(`user_topic_preferences table exists: ${tableCheck.rows[0].exists}`);
    ...
    `);
    console.log('\nPosts by tag:');
    tagStats.rows.forEach(r => console.log(`  ${r.tag}: ${r.count}`));
    ...
    const newPosts = await client.query(`
      ...
      ORDER BY slug
    `);
    console.log('\nDiabetes/Obesity posts:');
    newPosts.rows.forEach(r => console.log(`  [${r.tags.join(', ')}] ${r.title}`));
