...
- **One attention item on Today (principle 7).** A single compact callout (one row, fixed area) at the top: a **light-grey card with a thin red border** (the red border alone signals severity — operator review iterated off earlier left-accent-bar / alarm-wash / alert-icon versions) holding up to **3 severe domain names as borderless, individually-tappable red text**, with a "+N" when more are severe. It NEVER stacks N cards (the pile-up principle 7 forbids); the other severe domains stay in the grid with their own pills. `severeDomains` returns ALL `alarm`-tint domains, reusing the cards' own `lookupInterpretation` + `tintClassForBand` so callout and card can never disagree. NO new clinical copy (the chip is the versioned domain name; the band pill is read via lookup); each chip → its domain detail where the full reading + any 988 path live. Chips use vertical `hitSlop` to reach the 48px tap floor without horizontal overlap. Grid NOT reordered. Markers (watch tint) never trigger it.
