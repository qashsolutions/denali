...
- **One attention item on Today (principle 7).** A single **compact** callout (one row, fixed area — operator review: no red left accent bar, just the severity-tinted border) for the most-severe domain. `severeDomains` returns ALL `alarm`-tint domains; the callout shows the first + a **"N other(s) is/are severe"** subtitle when more exist — it NEVER stacks N cards (the pile-up principle 7 forbids); the other severe domains stay in the grid with their own pills. Pure helper reusing the cards' own `lookupInterpretation` + `tintClassForBand` so callout and card can never disagree. NO new clinical copy (reuses the versioned band pill); tap → the domain detail where the full reading + any 988 path live. Grid NOT reordered. Markers (watch tint) never trigger it.
