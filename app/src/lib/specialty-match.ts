...
/**
 * Check if a specialty is typically a referring provider
 * (can order most procedures but may need referral for specialty care)
 * @param specialty - The provider's specialty
 * @returns boolean
 */
export function isReferringSpecialty(specialty: string): boolean {
  ...
  return referringKeywords.some((keyword) => specialtyLower.includes(keyword));
}

/**
 * Get a plain English explanation of why specialty matters
 * @param procedure - The procedure being ordered
 * @returns Explanation string
 */
export function getSpecialtyExplanation(procedure: string): string {
  const procedureLower = procedure.toLowerCase();

  if (procedureLower.includes("mri") || procedureLower.includes("ct")) {
    return "Medicare expects imaging orders to come from providers who can demonstrate medical necessity based on their expertise. While your primary care doctor can order imaging, having a specialist order it may strengthen the case for approval.";
  }

  if (procedureLower.includes("surgery") || procedureLower.includes("replacement")) {
    return "Surgeries must be ordered by appropriately credentialed surgeons. Medicare verifies that the ordering provider has the specialty training to perform or oversee the procedure.";
  }

  if (procedureLower.includes("therapy")) {
    return "Physical and occupational therapy can be ordered by most physicians and some qualified practitioners. The key is that the ordering provider has evaluated the patient and can document the medical necessity.";
  }

  return "Medicare reviews whether the ordering provider's specialty is appropriate for the procedure being requested. Having the right specialist involved can help ensure smoother approval.";
}
