...
// Groups expanded by default; older groups start collapsed
const DEFAULT_EXPANDED = new Set(["Today", "Yesterday", "Past Week"]);
                ...
                return (
                  <div key={group}>
                    <button
                      ...
                      className="w-full flex items-center justify-between px-2 py-1.5 rounded hover:bg-[var(--bg-tertiary)] transition-colors group"
                      aria-expanded={isExpanded}
                    >
                      <span className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">
                        {group}
                        <span className="ml-1.5 text-[var(--text-muted)] font-normal normal-case tracking-normal">
                          ({items.length})
                        </span>
                      </span>
                      <ChevronIcon
                        className={`w-3.5 h-3.5 text-[var(--text-muted)] transition-transform duration-150 ${isExpanded ? "rotate-0" : "-rotate-90"}`}
                      />
                    </button>
                    {isExpanded && (
