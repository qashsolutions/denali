...
/**
 * POST /api/profile/birth-year-reminder/disable — Unit Tests
 *
 * The disable endpoint sets users.birth_year_modal_disabled = true
 * and fires a BIRTH_YEAR_REMINDER_DISABLED audit log entry.
 *
...
/** Minimal valid POST request to the disable endpoint */
function makeRequest(): NextRequest {
  return new NextRequest(
    "http://localhost/api/profile/birth-year-reminder/disable",
    { method: "POST" },
  );
}
...
describe("POST /api/profile/birth-year-reminder/disable", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("calls query() with the correct UPDATE SQL and user id", async () => {
    ...
    // The SQL must touch birth_year_modal_disabled and updated_at
  ...
  it("fires logAudit with BIRTH_YEAR_REMINDER_DISABLED action on success", async () => {
    await POST(makeRequest());
    ...
    expect(action).toBe("BIRTH_YEAR_REMINDER_DISABLED");
    expect(opts.userId).toBe(MOCK_USER.userId);
    expect(opts.resourceType).toBe("account");
    expect(opts.metadata).toMatchObject({ source: "modal_or_settings" });
  });
